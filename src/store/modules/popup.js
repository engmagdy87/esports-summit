...
const getters = {
  [types.popups.getters.GET_POPUP]: currentState => placeName => {
    ...
    const isPlaceExists =
      currentState.randomPopupData.places.filter(
...
const mutations = {
  [types.popups.mutations.SET_DASHBOARD_POPUPS_DATA]: (
    currentState,
    payload
  ) => {
    currentState.dashboardPopupsData = payload;
  },
  [types.popups.mutations.SET_IS_DASHBOARD_POPUPS_DATA_FETCHED]: (
    currentState,
    payload
  ) => {
  ...
  [types.popups.mutations.SET_IS_RANDOM_POPUP_DATA_FETCHED]: (
    currentState,
    payload
  ) => {
...
const getRandomPopupData = async ({ commit }) => {
  ...
  const response = await getRandomPopup()
    .then(response => {
      commit(
        types.popups.mutations.SET_RANDOM_POPUP_DATA,
        response.data.popup.length === 0 ? {} : response.data.popup[0]
      );
...
const getPopupsDataForDashboard = async ({ commit }) => {
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await getPopupsForDashboard()
    .then(response => {
      commit(
        types.popups.mutations.SET_DASHBOARD_POPUPS_DATA,
...
const deletePopup = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removePopup(popupId)
    .then(() => {
...
const deletePopupImage = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removePopupImage(popupId, imageId)
    .then(() => {
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
      return true;
    })
