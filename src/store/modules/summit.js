...
const mutations = {
  ...
  [types.summits.mutations.SET_IS_SUMMITS_HISTORY_FETCHED]: (
    currentState,
    payload
  ) => {
    currentState.isSummitsHistoryDataFetched = payload;
  },
  [types.summits.mutations.SET_IS_SUMMIT_DETAILS_FETCHED]: (
    currentState,
    payload
  ) => {
  ...
  [types.summits.mutations.SET_IS_SUMMITS_LIST_FETCHED]: (
    currentState,
    payload
  ) => {
  ...
  [types.summits.mutations.SET_COVER_HOME_EVENTS_IMAGE]: (
    currentState,
    payload
  ) => {
    currentState.coverHomeEventsImage = payload;
  },
  [types.summits.mutations.SET_COVER_CONTACT_US_IMAGE]: (
    currentState,
    payload
  ) => {
    currentState.coverContactUsImage = payload;
  },
  [types.summits.mutations.SET_COVER_TESTIMONIALS_IMAGE]: (
    currentState,
    payload
  ) => {
    currentState.coverTestimonialsImage = payload;
  },
  [types.summits.mutations.SET_COVER_GIVEAWAYS_IMAGE]: (
    currentState,
    payload
  ) => {
    currentState.coverGiveawaysImage = payload;
  },
  [types.summits.mutations.IS_COVER_HOME_EVENTS_IMAGE_FETCHED]: (
    currentState,
    payload
  ) => {
    currentState.isCoverHomeEventsImageFetched = payload;
  },
  [types.summits.mutations.IS_COVER_CONTACT_US_IMAGE_FETCHED]: (
    currentState,
    payload
  ) => {
    currentState.isCoverContactUsImageFetched = payload;
  },
  [types.summits.mutations.IS_COVER_TESTIMONIALS_IMAGE_FETCHED]: (
    currentState,
    payload
  ) => {
    currentState.isCoverTestimonialsImageFetched = payload;
  },
  [types.summits.mutations.IS_COVER_GIVEAWAYS_IMAGE_FETCHED]: (
    currentState,
    payload
  ) => {
...
const getSummitsListSummary = async ({ commit }) => {
  const response = await getSummitsList()
    .then(response => {
      commit(
        types.summits.mutations.SET_SUMMITS_LIST_DATA,
        response.data.summits
      );
...
const getSummitsHistoryData = async ({ commit }) => {
  const response = await getSummitsHistory()
    .then(response => {
      commit(
        types.summits.mutations.SET_SUMMITS_HISTORY,
        response.data.data.summit
      );
...
const getSummitDetailsData = async ({ commit }, summitId) => {
  const response = await getSummitDetails(summitId)
    .then(response => {
      commit(
        types.summits.mutations.SET_SUMMIT_DETAILS,
        response.data.data.summit
      );
...
const fetchCoverHomeEventsImage = async ({ commit }) => {
  const response = await getCoverHomeEventsImage()
    .then(response => {
      commit(
        types.summits.mutations.SET_COVER_HOME_EVENTS_IMAGE,
        response.data.cover
...
const fetchCoverContactUsImage = async ({ commit }) => {
  const response = await getCoverContactUsImage()
    .then(response => {
      commit(
        types.summits.mutations.SET_COVER_CONTACT_US_IMAGE,
        response.data.cover
...
const fetchCoverTestimonialsImage = async ({ commit }) => {
  const response = await getCoverTestimonialsImage()
    .then(response => {
      commit(
        types.summits.mutations.SET_COVER_TESTIMONIALS_IMAGE,
        response.data.cover
...
const fetchCoverGiveawaysImage = async ({ commit }) => {
  const response = await getCoverGiveawaysImage()
    .then(response => {
      commit(
        types.summits.mutations.SET_COVER_GIVEAWAYS_IMAGE,
        response.data.cover
...
const deleteSummit = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removeSummit(summitId)
    .then(() => {
      commit(
        types.summits.mutations.REMOVE_DELETED_SUMMIT,
        locationInDataArray
      );
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
      return true;
...
const deleteSummitImage = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removeSummitImage(summitId, imageId)
    .then(() => {
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
      return true;
    })
...
const deleteSummitVideo = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removeSummitVideo(summitId, videoId)
    .then(() => {
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
      return true;
    })
...
const actions = {
  ...
  [types.summits.actions
    .FETCH_COVER_HOME_EVENTS_IMAGE]: fetchCoverHomeEventsImage,
  [types.summits.actions
    .FETCH_COVER_CONTACT_US_IMAGE]: fetchCoverContactUsImage,
  [types.summits.actions
    .FETCH_COVER_TESTIMONIALS_IMAGE]: fetchCoverTestimonialsImage,
