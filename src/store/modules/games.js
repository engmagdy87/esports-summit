...
const mutations = {
  ...
  [types.games.mutations.SET_IS_GAMES_DATA_FETCHED]: (
    currentState,
    payload
  ) => {
  ...
  [types.games.mutations.SET_IS_DASHBOARD_GAMES_DATA_FETCHED]: (
    currentState,
    payload
  ) => {
    currentState.isDashboardGamesDataFetched = payload;
  },
  [types.games.mutations.SET_IS_GAMES_CARDS_VIEW_FETCHED]: (
    currentState,
    payload
  ) => {
  ...
  [types.games.mutations.SET_IS_GAME_DETAILS_DATA_FETCHED]: (
    currentState,
    payload
  ) => {
...
const getGamesDataForDashboard = async ({ commit }) => {
  const response = await getGamesForDashboard()
    .then(response => {
      commit(
        types.games.mutations.SET_DASHBOARD_GAMES_DATA,
        response.data.games
      );
...
const getGamesCardsViewData = async ({ commit }) => {
  const response = await getGamesCardsView()
    .then(response => {
...
const getGameHistoryDetailsData = async ({ commit }, payload) => {
  const { summitId, gameId } = payload;
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await getGameHistoryDetails(summitId, gameId)
    .then(response => {
      commit(types.games.mutations.SET_GAME_DETAILS_DATA, response.data.game);
      commit(types.games.mutations.SET_IS_GAME_DETAILS_DATA_FETCHED, true);
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
...
const deleteGame = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removeGame(gameId)
    .then(() => {
...
const deleteGameImage = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await removeGameImage(gameId, imageId)
    .then(() => {
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
      return true;
    })
...
const editGameData = async ({ commit }, payload) => {
  ...
  commit(types.home.mutations.SET_SPINNER_FLAG, true);
  const response = await editGame(gameId, data)
    .then(() => {
      commit(types.home.mutations.SET_SPINNER_FLAG, false);
      return true;
    })
...
const actions = {
  ...
  [types.games.actions.FETCH_GAME_HISTORY_DETAILS]: getGameHistoryDetailsData,
